    ...
    public List<Order> parseCSV(MultipartFile file) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream()))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String[] fields = line.split(","); // Changed delimiter from comma to tab
                if (fields.length < 6) {
                    // Log or handle malformed line
                    System.err.println("Skipping malformed CSV line (expected at least 6 tab-separated fields): " + line);
                    continue;
                }
                ...
                try {
                    ...
                    LocalDateTime arriveDate = parseCustomDateTimeString(fields[0].trim());
                    ...
                    double glpRequest = Double.parseDouble(fields[4].trim());
                    long dueHours = Long.parseLong(fields[5].trim());
                    LocalDateTime dueDate = arriveDate.plusHours(dueHours);

                    // Using the Order constructor: Order(String id, LocalDateTime arriveDate, LocalDateTime dueDate, double GLPRequest, Position position)
                    // This constructor also sets remainingGLP = GLPRequest.
                    // ID is passed as null, to be generated by the database.
                    Order order = new Order(null, arriveDate, dueDate, glpRequest, position);
                    
                    // Note: The OrderStatus is not set here as the provided Order class
                    // does not have a public setter for status or a constructor argument for it.
                    // It will default to null or whatever the Order class initializes it to.
                    // If OrderStatus.PENDING is desired, the Order class would need modification
                    // or a default initialization.

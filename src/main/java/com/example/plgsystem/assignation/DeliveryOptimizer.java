...
/**
 * Utility class to optimize delivery assignments by applying split and merge operations
 */
public class DeliveryOptimizer {
    ...
    public static Map<String, List<DeliveryPart>> optimizeAssignments(
        ...
        for (String vehicleId : assignments.keySet()) {
            ...
            // 3. Merge optimally based on capacity
            // System.out.println("Merging parts for vehicle: " + vehicleId + " with capacity: " + capacity);
            // StringBuilder sb = new StringBuilder();
            // int i=0;
            // for (DeliveryPart part : atomicParts) {
            //     sb.append("Part " + (i++) + ": " + part).append("\n");
            // }
            // System.out.println("Atomic parts: \n" + sb.toString());
            List<DeliveryPart> mergedParts = DeliveryConsolidator.mergeLikeRLE(atomicParts, capacity);
            // StringBuilder sb2 = new StringBuilder();
            // i = 0;
            // for (DeliveryPart part : mergedParts) {
            //     sb2.append("Merged Part " + (i++) + ": " + part).append("\n");
            // }
    ...
    /**
     * Move a delivery part from one vehicle to another with proper splitting/merging
     * 
     ...
     * @param deliveryIndexToMove Index of the delivery to move in the source vehicle's list

...
public class Action {
    ...
    /**
     * Checks if this action can be executed at the current time
     * 
     * @param vehicle The vehicle that would perform the action
     * @param environment The simulation environment
     * @param currentTime The current simulation time
     * @return true if the action can be executed, false otherwise
     */
    public boolean canExecute(Vehicle vehicle, SimulationState environment, LocalDateTime currentTime) {
        if (currentTime.isBefore(expectedStartTime)) {
            return false;
        }
        
        // Check if vehicle is at the right position for non-driving actions
        if (type != ActionType.DRIVE && !vehicle.getCurrentPosition().equals(destination)) {
            return false;
        }
    ...
    /**
     * Estimates the completion time of this action based on the current state
     * 
     * @param vehicle The vehicle performing the action
     * @param environment The simulation environment
     * @param startTime The time when the action would start
     * @return The estimated completion time
     */
    public LocalDateTime estimateCompletionTime(Vehicle vehicle, SimulationState environment, LocalDateTime startTime) {
        // For simplicity, we'll use the expected duration
     ...
     */
    private Depot findDepotByPosition(SimulationState environment, Position position) {
